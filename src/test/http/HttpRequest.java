...
import model.Udp_model.enums.requestType;
...
public class HttpRequest {
	...
	public void translateAll(ObservableList<StringModel> stringModels) {
		Platform.runLater(new Runnable() {
			@Override
			public void run() {
				for (int i = 0; i < stringModels.size(); i++) {
					new Thread(new AddThread(HttpRequest.this, stringModels
							.get(i))).start();
				}
			}
		});
	...
	class AddThread extends Thread {
		private HttpRequest plate;
		private StringModel egg;

		public AddThread(HttpRequest plate, StringModel stringModel) {
			this.plate = plate;
	...
	/** 放鸡蛋 （加入子线程） */
	public void putEgg(StringModel egg) {
		try {
			if(egg!=null&&egg.getValueEN()==null||egg.getValueEN().isEmpty()&&egg.getValueCH()!=null){
			String result = sendRequest(RequestType.POST, egg.getValueCH());
