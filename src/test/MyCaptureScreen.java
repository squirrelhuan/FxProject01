...
public class MyCaptureScreen extends JWindow {

	private static Test test;
	private BufferedImage imageView;
	
	private ToolsWindow tools = null;// 工具栏
	private BufferedImage image_buf;
	private BufferedImage image = null;
	private BufferedImage tempImage = null;// 黑色背景
	private BufferedImage saveImage = null;
	private static MyCaptureScreen myCaptureScreen;
	private static Temp temp;

/*	private static MyCaptureScreen myCaptureScreen = new MyCaptureScreen();

	public static MyCaptureScreen getInstance(Test test) {
		mtest = test;
		return myCaptureScreen;
	}*/

	// 构造函数
	public MyCaptureScreen(Test test) {
		this.test = test;
		this.myCaptureScreen = this;
		doStart();
	}
	...
	private class Temp extends JPanel implements MouseListener,
		...
		public void mouseReleased(MouseEvent me) {
			if (isDraged) {
				...
				if (tools == null) {
					if (max1 == top) {// top
						tools = new ToolsWindow(MyCaptureScreen.this, Math.max(
								startX, endX), Math.min(startY, endY));
						tools.setLocation(
								Math.max(startX, endX) - tools.getWidth(),
								Math.min(startY, endY) - tools.getHeight()-1);
					} else {
						tools = new ToolsWindow(MyCaptureScreen.this, Math.max(
								startX, endX), Math.max(startY, endY));
						tools.setLocation(
								Math.max(startX, endX) - tools.getWidth(),
	...
	class ToolsWindow extends JWindow {
		private MyCaptureScreen parent;
		private ToolsWindow toolsWindow;
		JToolBar toolBar1;

		public ToolsWindow(MyCaptureScreen captureScreen, int x, int y) {
				...
				public void actionPerformed(ActionEvent e) {
					...
					Platform.runLater(new Runnable(){
	                    ...
	                    public void run() {
	                    	temp.jf.dispose();
	                    	toolsWindow.dispose();
	                    	CaptureScreen_listener captureScreen_listener = CaptureScreen_listener.getinstence();
	    					captureScreen_listener.imageView=saveImage;
	                    	CaptureScreen_Tab tab = new CaptureScreen_Tab("截图",test);
	    					test.tabPane_01.getTabs().add(tab);
	    					test.tabPane_01.getSelectionModel().select(test.tabPane_01.getTabs().size()-1);
	...
	private void updates(){
        ...
        /*if(get!=null){
            ...
            PicPanel pic=new PicPanel(imageView);
